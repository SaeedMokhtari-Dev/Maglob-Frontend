...
export default class EditMenuViewModel
{
    ...
    detailMenuResponse: DetailMenuResponse = new DetailMenuResponse();
    addMenuRequest: AddMenuRequest = new AddMenuRequest();
    editMenuRequest: EditMenuRequest = new EditMenuRequest();
    ...
    public async addMenu(request: AddMenuRequest)
    {
        try
        {
            ...
            if(response && response.success)
            {
                ...
                /*await this.menusStore.getMenuViewModel.getAllMenus(new GetMenusRequest(20, 0));*/
            }
            else{
                this.errorMessage = getLocalizedString(response.message);
            }
